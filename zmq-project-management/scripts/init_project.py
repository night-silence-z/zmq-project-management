...
默认只创建有内容的 README、台账和项目自举指针。输入、产出、验收与归档目录
...
def main():
    ...
    parser.add_argument(
        ...
        default="both",
        help="写入项目自举指针，默认同时写 AGENTS.md 与 CLAUDE.md",
    )
    parser.add_argument(
        "--no-bootstrap",
        action="store_true",
    ...
    root = pathlib.Path(args.base).resolve() / args.name
    ...
    for path in created:
        print(f"  {path}")
    print("下一步：收敛产出物、受众与用途、可观察验收标准；普通执行默认走轻量日常迭代。")
    return 0
